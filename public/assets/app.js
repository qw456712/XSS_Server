  ...
  eventRows.innerHTML = events.map((event) => {
    ...
    return `
      <tr>
        ...
        <td
          class="page"
          title="${escapeHtml(event.page || '')}"
        >
          ${escapeHtml(event.page || '-')}
        </td>

        <td
          class="page"
          title="${escapeHtml(event.referrer || '')}"
        >
          ${escapeHtml(event.referrer || '-')}
        </td>

        <td
          class="page"
          title="${escapeHtml(event.userAgent || '')}"
        >
          ${escapeHtml(event.userAgent || '-')}
        </td>
      </tr>
    `;
  }).join('');
...
navigationLinks.forEach((link) => {
  link.addEventListener('click', (event) => {
    ...
    if (!target) {
      console.error(
        `Navigation target not found: ${targetSelector}`
      );
